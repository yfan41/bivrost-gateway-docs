#!/usr/bin/env node
/**
 * Render the whole manual to one PDF per language.
 *
 ...
 *
 *   pnpm build && pnpm pdf
 *   DOCS_BASE=/gateway pnpm build && DOCS_BASE=/gateway pnpm pdf
 */
...
const targets = [
  ...
  },
  {
    locale: 'en',
    path: `${base}/en/print/`,
    title: 'Bivrost IoT Gateway User Manual',
  },
];

if (neutral) {
  targets.splice(1);
  targets[0].title = '物联网关 说明书';
}
...
// --- serve dist/ --------------------------------------------------------------
// Not file://: every screenshot is a root-absolute /img/manual/... path (rebased to
// /gateway/img/... under a base), which would resolve to file:///img/ and 404.
...
try {
  for (const { locale, path, title } of targets) {
    ...
    const neutralReport = neutral ? await neutralExport.prepareNeutralPage(page, root, 'manual', version) : null;
    ...
    const body = await page.pdf({
      ...
      // A real bookmark tree for a 23-chapter manual; `outline` requires `tagged`.
      tagged: true,
      outline: true,
    });
    ...
    if (neutral) {
      ...
      if (bytes.length < 500_000) throw new Error('中性 PDF 异常偏小');
