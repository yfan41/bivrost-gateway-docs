...
const targets = [
  { locale: 'zh-CN', path: `${base}/print/`, title: '彼络物联网关 说明书' },
  { locale: 'en', path: `${base}/en/print/`, title: 'Bivrost IoT Gateway Manual' },
];
...
try {
  for (const { locale, path, title } of targets) {
    ...
    await page.pdf({
      ...
      headerTemplate: `<div style="${chrome} text-align:center;">Bivrost IoT Gateway Manual · v${version}</div>`,
