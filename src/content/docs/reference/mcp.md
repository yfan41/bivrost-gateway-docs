...
MCP 服务**默认关闭**，关闭时访问 `/mcp` 返回 HTTP 404。在[3.6.8. MCP 设置](/usage/communication/#mcp)中找到 MCP 项，打开开关并保存即可启用，立即生效，无需重启网关或服务。
...
云平台的 MCP 服务同样默认关闭，需由云平台运维在其 appsettings 中设置 `EnableMcp: true` 并重启服务后才可访问。
